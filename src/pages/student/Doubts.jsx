...
export default function StudentDoubts() {
  ...
  return (
    <div>
      ...
      <h2 className="text-lg font-bold text-white mb-4">Your Doubts</h2>
      {loading ? (
        ...
        <div className="space-y-4">
          {doubts.map(d => (
            <div key={d.id} className="bg-[#111111] rounded-2xl p-5 border border-slate-800">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    {d.subject && <span className="badge badge-navy text-xs">{d.subject}</span>}
                    <span className={`badge ${statusColors[d.status] || 'badge-navy'}`}>{d.status}</span>
                  </div>
                </div>
              </div>
              ...
              {d.adminReply && (
                <div className="mt-3 p-3 rounded-xl bg-green-brand/5 border border-green-brand/20">
                  <p className="text-xs font-bold text-green-brand uppercase mb-1">Admin Reply</p>
