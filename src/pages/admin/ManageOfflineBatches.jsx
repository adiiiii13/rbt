...
export default function ManageOfflineBatches() {
  ...
  // Filter offline batches
  const offlineBatches = useMemo(() =>
    batches.filter(b => b.type === 'offline' || b.isOffline),
    [batches]
  )

  // Get students assigned to offline batches
  const offlineStudentIds = useMemo(() => {
    const ids = new Set()
    students.forEach(s => {
      if (s.assignedBatchId && offlineBatches.some(b => b.id === s.assignedBatchId)) {
        ids.add(s.id)
      }
    })
    return ids
  }, [students, offlineBatches])
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Offline Batches</h1>
          <p className="text-sm text-slate-400">Manage offline/in-person batches and assign students manually</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {[
          { id: 'batches', label: 'Batches', count: offlineBatches.length },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {offlineBatches.length === 0 ? (
            <div className="col-span-full bg-[#111111] rounded-2xl border border-slate-800 p-12 text-center">
              <p className="text-slate-500 mb-3">No offline batches yet.</p>
            ...
            <div key={b.id} className="bg-[#111111] rounded-2xl border border-slate-800 p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs font-bold px-2 py-0.5 rounded bg-amber-500/10 text-amber-400">Offline</span>
              </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-800">
                  {offlineStudents.map(s => {
                    const batch = offlineBatches.find(b => b.id === s.assignedBatchId)
                    return (
                      ...
                      <td>
                        {offlineBatches.length > 0 ? (
                          <button onClick={() => {
                            ...
                            setAssignModal(offlineBatches[0])
                            setSelectedStudent('')
      ...
      <Modal isOpen={batchModal} onClose={() => { setBatchModal(false); setEditing(null) }} title={editing ? 'Edit Batch' : 'Create Offline Batch'}>
